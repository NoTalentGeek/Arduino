...
//A function to control the push button.
...
void loop(){
	...
	while(vs1053.playingMusic){
		isButtonPlayStop		= IsButtonPressed(isButtonPlayStop, PLAY_STOP, &Stop);
	...
	while(!vs1053.playingMusic){
		isButtonPlayStop		= IsButtonPressed(isButtonPlayStop, mp3IndexBufferCharArray, PLAY_STOP, &Play);
		isButtonPauseUnpause	= IsButtonPressed(isButtonPauseUnpause, vs1053.paused(), PAUSE_UNPAUSE, &PauseUnpause);
		isButtonNext			= IsButtonPressed(isButtonNext, true, NEXT, &NextPrevious);
		isButtonPrevious		= IsButtonPressed(isButtonPrevious, false, PREVIOUS, &NextPrevious);
		isButtonVolUp			= IsButtonPressed(isButtonVolUp, VOL_UP, &VolUp);
...
void PhysicalButton(bool _isPlayingMusic){
	...
	else if(isButtonPauseUnpause){
		PauseUnpause(vs1053.paused());
	}
	else if(isButtonNext){ NextPrevious(true); }
	else if(isButtonPrevious){ NextPrevious(false); }
	else if(isButtonVolUp){ /*PENDING: Volume button.*/ }
	else if(isButtonVolDown){ /*PENDING: Volume button.*/ }
