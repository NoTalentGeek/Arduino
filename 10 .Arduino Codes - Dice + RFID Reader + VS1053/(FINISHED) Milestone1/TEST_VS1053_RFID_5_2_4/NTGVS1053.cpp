...
char explanationNumber[] 	= "31.mp3";
char pleaseVisitAndTap[] 	= "32.mp3";
char prime[] 				= "33.mp3";
char welcomeToExhibition[] 	= "34.mp3";
char welcomeToMuseumX[] 	= "35.mp3";
char youNeedToGo[] 			= "36.mp3";
...
void NTGVS1053::NTGVS1053Setup(){
	...
	Play(welcomeToMuseumX); //Welcome to museum X.
	Play(pleaseVisitAndTap); //Please visit and tap into exhibition.
	Play(prime); //Prime.

}

bool haveVisitedTargetExhibition = false;
int targetExhibitionIndex = 0;
...
void NTGVS1053::NTGVS1053Loop(){
	...
	while(!vs1053.playingMusic){
		...
		if(mp3Index != -1){
			...
			if(mp3Index == targetExhibitionIndex){ haveVisitedTargetExhibition = true; }
			else if(mp3Index != targetExhibitionIndex){ haveVisitedTargetExhibition = false; }

			Play(welcomeToExhibition);
			Play(mp3IndexBufferCharArray);

			if(haveVisitedTargetExhibition){
				//Give explanation here.
				//Roll and assign new target here.
				targetExhibitionIndex ++;
				haveVisitedTargetExhibition = false;
			}
			else if(!haveVisitedTargetExhibition){	
				Play(pleaseVisitAndTap);
				if(targetExhibitionIndex == 0){
					Play(prime);
				}
				else if(targetExhibitionIndex != 0){
					mp3IndexBufferString = String(targetExhibitionIndex) + ".mp3";
					mp3IndexBufferString.toCharArray(mp3IndexBufferCharArray, 50);
					Play(mp3IndexBufferCharArray);
				}
			}
