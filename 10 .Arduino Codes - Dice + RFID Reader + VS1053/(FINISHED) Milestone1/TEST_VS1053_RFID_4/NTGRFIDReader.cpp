...
void NTGRFIDReader::NTGRFIDReaderLoop(){
	...
	/*
	if((softwareSerial.available() > bytesBrutto || softwareSerial.available() < bytesBrutto) && softwareSerial.available() > 0){
		int test1 = softwareSerial.available();
		Serial.println(test1);
		//int test2 = softwareSerial.read();
		//Serial.println(test2);
	}
	*/

	//If there is a tags within radius of RFID reader set the tagRead to true.
	if(softwareSerial.available() == bytesBrutto){ Serial.println("TEST"); tagRead = true; }
	if(tagRead){
